...
[
  ...
  turtle-to-offspring          ;; total wealth kept aside for offspring
  inheritance      ;; wealth left after tax
  wealth-tax       ;; part of inheritance lost due to taxation
]
...
to setup
  ...
  ;; =========================================================================EXTENSION 3===========================================================================================================================
  ;; Introducing seasons to variate the availability of grain
  ;; Monsoon - heavy rainfall can lead to crop failure - hence least amount of grain is found in the patches = 20
  ;; Summer - hot climatic conditions aren't very ideal for crop growth - moderate amount of grain is found in the patches = 45
  ;; Spring - best season for crop growth - maximum amount of grain available  = 80
  ;;================================================================================================================================================================================================================
  ifelse summer?
    [ set max-grain 45 ]
    [ ifelse monsoon?
...
to set-initial-turtle-vars
  ...
  ;; =========================================================================EXTENSION 1===========================================================================================================================
  ;; Equal initial distribution of wealth-------------------------------------------------------------------------------------------------------------------------------------------------------------
  ;;    set wealth metabolism + 50
  ;; Random intial distribution of wealth------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  ;; =========================================================================EXTENSION 4===========================================================================================================================
  ;; Wealth Inheritance by Offsprings + Inheritance Taxes
  ;; When a turtle dies, it transfers its wealth to its offspring
  ;; Wealth gets accumulated with every generation
  ;; Progressive inheritance tax - tax rate adjustable
  ;; turtle-to-offspring - taxrate * turtle-to-offspring = inheritance
  ;; total-wealth-tax * ( 1 - (wealth / sum [ wealth ] of turtles)) -> every turtle's tax depends on wealth at birth
  ;;================================================================================================================================================================================================================
  ;;set wealth ifelse-value (sum [ wealth ] of turtles > 0)
    ;;[ metabolism + inheritance + total-wealth-tax * ( 1 - (wealth / sum [ wealth ] of turtles))]
    ;;[ metabolism + inheritance + total-wealth-tax / num-people ]
...
;; As the model progresses at every instance, income tax is imposed thereby chaning the turtle's wealth and color
;; Rich - 30% of their wealth (blue)
;; Middle Class - 15% of their wealth (green)
;; Poor - 5% of their wealth (red)
;; ===============================================================================================================================================================================================================
...
to recolor-turtles
  ...
  ask turtles [
    ...
    ;; Wealth Change
    ifelse (color = red)
    [ set wealth wealth * 95 / 100 ]
    [ ifelse (color = green)
      [ set wealth wealth * 85 / 100 ]
      [ set wealth wealth * 70 / 100 ]] ]
end
...
to move-eat-age-die  ;; turtle procedure
  ...
  if (wealth < 0) or (age >= life-expectancy)
    [ ;;set turtle-to-offspring max list 0 wealth
      ;;set wealth-tax tax-rate * turtle-to-offspring
      ;;set inheritance turtle-to-offspring - wealth-tax
      set-initial-turtle-vars ]
end
...
to update-lorenz-and-gini
  ...
  repeat num-people [
    ...
    set total-wealth-in-economy wealth-sum-so-far
...
;;to update-aggregates
  ;;set total-wealth-tax sum [wealth-tax] of turtles
;; end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot total-wealth-in-economy"
...
1428
178
% Wealth with Lower Class
(sum [wealth] of turtles with [color = red]) /( sum [wealth] of turtles with [color = red] + sum [wealth] of turtles with [color = blue] + sum [wealth] of turtles with [color = green])* 100
5
1
11
...
1429
243
% Wealth with Middle Class
(sum [wealth] of turtles with [color = green]) /( sum [wealth] of turtles with [color = red] + sum [wealth] of turtles with [color = blue] + sum [wealth] of turtles with [color = green]) * 100
5
1
11
...
1431
316
% Wealth with Upper Class
(sum [wealth] of turtles with [color = blue]) /( sum [wealth] of turtles with [color = red] + sum [wealth] of turtles with [color = blue] + sum [wealth] of turtles with [color = green]) * 100
5
1
11
...
1. Run the model for different values of NUM-GRAIN_GROWN, LIFE-EXPECTANCY_MAX, initial location, GRAIN-GROWTH-INTERVAL and notice the change in the output. Also, introduce a random initial distribution of wealth among the people.


3. Introduce government policies such as a progressive taxation system inorder to redistirbute the income between the rich and the poor.

4. Introduce seasons to vary the availability of grain in different regions of the landscape.


5. Study intergenerational mobility by allowing the offspring to inherit wealth from the parent turtle. Impose an inheritance tax on the same.

Use these policies to achieve a state of equality in the wealth distribution, if possible.
